...
fw.describe("MiniFader - fading", function()
	...
	fw.it("brings back only the bar a hovered button sits on", function()
		...
		-- the mouse lands on a button rather than the bar behind it, so the button has to
	...
	fw.it("keeps a bar the mouse is on visible when combat ends", function()
		local context = harness.Load("MiniFader")
		...
		-- the hover starts while combat holds the bars up, so nothing is fading yet
		MultiBarBottomLeft:GetScript("OnEnter")(MultiBarBottomLeft)
